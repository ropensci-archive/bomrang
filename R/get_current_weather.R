...
#' @param latlon A length-2 numeric vector. When given instead of \code{station_name}, the nearest station
#' (in this package) is used, with a message indicating the nearest such station. (See also \code{\link{sweep_for_stations}}.)
#' Ignored if used in combination with
...
#' @param emit_latlon_msg Logical. If \code{TRUE} (the default), and \code{latlon} is selected,
#' a message is emitted before the table is returned indicating which station was actually used
#' (i.e. which station was found to be nearest to the given coordinate).
...
get_current_weather <- function(station_name, latlon = NULL, raw = FALSE, emit_latlon_msg = TRUE) {
  ...
  if (!missing(station_name)) {
    ...
    json_url <- JSONurl_latlon_by_station_name[name == the_station_name][["url"]]
    ...
    on.exit(message("Using station_name = '", station_nrst_latlon$name,
                    "', at latitude = ", station_nrst_latlon$Lat, ", ",
                    "longitude = ", station_nrst_latlon$Lon))
  ...
  double_cols <- c("lat", "lon", "apparent_t", "cloud_base_m", "cloud_oktas", "rain_trace")
  # (i.e. not raw)
  cook <- function(DT) {
    DTnoms <- names(DT)
