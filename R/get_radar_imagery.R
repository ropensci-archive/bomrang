...
#' Suggested to check file availability first by using \code{\link{get_available_radar}}.
#'
...
get_radar_imagery <- function(product_id = NULL) {
  ...
  if (length(product_id) != 1) {
    stop("\nbomrang only supports working with one Product ID at a time for radar images\n")
  }
  ftp_base <- "ftp://ftp.bom.gov.au/anon/gen/radar"
