...
There are weather stations that do have a WMO but don't report online, _e.g._, KIRIBATI NTC AWS or MARSHALL ISLANDS NTC AWS, in this section remove these from the list and then create a database to provide URLs for valid JSON files providing weather data from BOM.
...
```{r changes-JSONurl-site-list}
load(system.file("extdata", "JSONurl_site_list.rda", package = "bomrang"))

(changes <-
    waldo::compare(new_JSONurl_site_list, JSONurl_site_list, max_diffs = Inf))
```
...
load(system.file("extdata", "JSONurl_site_list_changes.rda",
                 package = "bomrang"))

release_version <- paste0("v", packageVersion("bomrang"))
JSONurl_site_list_changes[[release_version]] <- changes
...
(changes <-
    waldo::compare(new_stations_site_list, stations_site_list, max_diffs = Inf))
```
...
load(system.file("extdata", "stations_site_list_changes.rda",
                 package = "bomrang"))

release_version <- paste0("v", packageVersion("bomrang"))
stations_site_list_changes[[release_version]] <- changes
