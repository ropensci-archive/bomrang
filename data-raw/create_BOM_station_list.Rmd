...
bom_locations <- dplyr::left_join(points, bom_locations)
```

## Recode corrected state values 
...
# recode the states to match product codes using corrected state values
# IDD - NT,
# IDN - NSW/ACT,
# IDQ - Qld,
# IDS - SA,
...
bom_locations$state_code[bom_locations$corrected_state == "QL"] <- "Q"
bom_locations$state_code[bom_locations$corrected_state == "VI"] <- "V"
bom_locations$state_code[bom_locations$corrected_state == "NT"] <- "D"
bom_locations$state_code[bom_locations$corrected_state == "TS" |
                              bom_locations$corrected_state == "ANT"] <- "T"
bom_locations$state_code[bom_locations$corrected_state == "NS"] <- "N"
bom_locations$state_code[bom_locations$corrected_state == "SA"] <- "S"

# replace original state values with corrected state values

bom_locations$state[bom_locations$corrected_state == "WA"] <- "WA"
bom_locations$state[bom_locations$corrected_state == "QL"] <- "QLD"
bom_locations$state[bom_locations$corrected_state == "VI"] <- "VIC"
bom_locations$state[bom_locations$corrected_state == "NT"] <- "NT"
bom_locations$state[bom_locations$corrected_state == "TS"] <- "TAS"
bom_locations$state[bom_locations$corrected_state == "NS"] <- "NSW"
bom_locations$state[bom_locations$corrected_state == "SA"] <- "SA"

stations_site_list <-
  bom_locations %>%
  dplyr::select(names(bom_stations_raw), ) %>%
  dplyr::mutate(
    url = dplyr::case_when(
      .$state != "ANT" & !is.na(.$wmo) ~
        paste0(
