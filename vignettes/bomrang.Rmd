...
each state and `get_historical_weather()` which fetches historical daily temperature
...
## Using get_historical_weather()

`get_historical_weather()` takes either of two arguments: `stationid` and `latlon`, as 
...
```r
(Canberra_mintemps <- get_historical_weather(latlon = c(-35.2809, 149.1300),
                                     type = "min"))
...
```r
adlmax <- get_historical_weather(stationid = "023000", type = "max")
