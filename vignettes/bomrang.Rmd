...
Several functions are provided by _bomrang_.The main functionality of _bomrang_
is provided through three functions, `get_precis_forecast()`, which retrieves
the précis (short) forecast; `get_current_weather()`, which fetches the current
weather from a given station; and `get_ag_bulletin()`, which retrieves the
agriculture bulletin. Other functions include `update_forecast_locations()`, 
which updates an internal database of forecast locations distributed with the
package, and `sweep_for_stations()` which returns the nearest weather stations
to a point in Australia.

## BoM weather stations

BoM maintains a webpage to help users select the proper weather station to meet
the needs of the type of data required.

<http://www.bom.gov.au/climate/data/stations/about-weather-station-data.shtml>

### Station locations

BoM obtains data from many different typs of observing stations around Australia
and Antarctica. The following map shows the station locations available for
querying in _bomrang_. Note that not all stations provide data for all requests.

```{r map_Australia_stations, eval=TRUE, echo=FALSE, message=FALSE, fig.width=7, fig.height=5}
load(system.file("extdata", "stations_site_list.rda",
                 package = "bomrang"))

stations_site_list <- stations_site_list[stations_site_list$lat > -50, ]

Oz <- rnaturalearth::ne_countries(country = "Australia", 
                                  scale = 110,
                                  returnclass = "sp")

Oz <- ggplot2::fortify(Oz)

ggplot2::ggplot() +
    ggplot2::geom_polygon(data = Oz,
                        ggplot2::aes(x = long, y = lat, group = group),
                        fill = "white", colour = "black") +
  ggplot2::geom_point(data = stations_site_list, 
  ...
  ggplot2::coord_map() +
  ggplot2::ggtitle("BoM Station Locations",
                   sub = "Australia, outlying islands and bouys")
rm(stations_site_list)
```

```{r map_Antarctica_stations, eval=TRUE, echo=FALSE, message=FALSE, fig.width=7, fig.height=5}
load(system.file("extdata", "stations_site_list.rda",
                 package = "bomrang"))

stations_site_list <- stations_site_list[stations_site_list$lat <= -50, ]

Antarctica <- rnaturalearth::ne_countries(country = "Antarctica", 
                                          scale = 110,
                                          returnclass = "sp")


Antarctica <- ggplot2::fortify(Antarctica)

ggplot2::ggplot() +
    ggplot2::geom_polygon(data = Antarctica,
                        ggplot2::aes(x = long, y = lat, group = group),
                        fill = "white", colour = "black") +
  ggplot2::geom_point(data = stations_site_list, 
                ggplot2::aes(x = lon, y = lat)) +
  ...
  ggplot2::theme_minimal() +
  ggplot2::ggtitle("BoM Station Locations",
                   sub = "Antarctica, outlying islands and bouys")
rm(stations_site_list)
```
...
that latitude-longitude coordinate. The function also emits a message, to tell
the user which station was used.
...
from the BoM server and updates _bomrang_'s internal database of forecast
locations. There is no need to use this unless you know that a forecast location
exists in a more current version of the BoM forecast location database that is
not available in the database distributed with _bomrang_. This database is
distributed with the package to make the process faster when fetching the
forecast.
...
<tr><td>Abbr[6]</td><td>An abbeviated name (normally 4 characters) used for the
station</td></tr>
...
<tr><td>wmo</td><td>World Meteorlogical Organization number (unique ID used
worldwide)</td></tr>
